...
int main() {
    ...
    uint32_t off = fread(blob_bytes, 1, datfile_sz, datfile);
    fclose(datfile);
	...
	off = (seek_indexes[i] % ((datfile_sz-CHUNK_SIZE+16)/ALIGN)) * ALIGN/sizeof(uint32_t);
	for(size_t j = 0; j < CHUNK_SIZE/sizeof(uint32_t); j++) {
	    *(p1_index + j) = ( *(p1_index + j) ^ *(blob_index + off + j) ) * 0x1000193;
    ...
    for(size_t i = 0; i < HASH_SIZE/sizeof(uint32_t); i++) *(hash_index+i) = *(p1_index + i*2) ^ *(p1_index + i*2 + 1);

    printf("hash: ");
    for(size_t i = 0; i < HASH_SIZE; i++) printf("%02X", hash[i]);
    printf("\n");
